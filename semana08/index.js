const config = require("./resemble.config.json");
...
const GHOSTV_A = getLastPart(pathImagesGhostV_A)
const GHOSTV_B = getLastPart(pathImagesGhostV_B)
...
function setFolders(pathImagesV4, pathImagesV5, resImagesPath){
  try {
    ...
    fs.cpSync(pathImagesV4, `${resImagesPath}/${GHOSTV_A}`, {recursive: true,});
    fs.cpSync(pathImagesV5, `${resImagesPath}/${GHOSTV_B}`, {recursive: true,});
...
async function ejecutarVrt(resImagesPath){
  const filesFolderV1 = fs.readdirSync(`${resImagesPath}/${GHOSTV_A}`);
  const filesFolderV2 = fs.readdirSync(`${resImagesPath}/${GHOSTV_B}`);
  
  let resultInfo = {}
  for(i in filesFolderV1){
    const screenshotFileName = filesFolderV1[i];
    const pathImageV1 = `${resImagesPath}/${GHOSTV_A}/${screenshotFileName}`;
    const pathImageV2 = `${resImagesPath}/${GHOSTV_B}/${screenshotFileName}`;
...
processVrt(pathImagesGhostV_A, pathImagesGhostV_B);
