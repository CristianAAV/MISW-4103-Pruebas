describe('Funcionalidad de publicar posts', () => {
    const ESCENARIO = 'post_con_hora_invalida';
    const SECCION = 'posts';
    let dataPool = {}
    ...
    it('Escenario 14 pseudoaleatorio: Crear post con hora invalida', () => {    
        ...
        cy.get(`.gh-date-time-picker-time `).clear();
        cy.finalizarPaso(`${ESCENARIO}_step06`);

        const posthora = dataPool.hora;
        cy.get(`.gh-date-time-picker-time `).type(posthora);
