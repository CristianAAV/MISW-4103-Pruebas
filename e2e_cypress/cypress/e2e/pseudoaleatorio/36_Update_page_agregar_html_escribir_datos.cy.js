describe('update pages y agegar campo markdown', () => {
    ...
    cy.get('p[data-koenig-dnd-droppable="true"] > span[data-lexical-text="true"]').first().click().type('{enter}');
     ...
     //escoger html
     cy.get('div.flex.w-full.justify-between > div:contains("HTML")').click();
     //editar texto del html
     cy.get('div[spellcheck="false"][autocorrect="off"][autocapitalize="off"][translate="no"][contenteditable="true"].cm-content.cm-lineWrapping[role="textbox"][aria-multiline="true"][data-language="html"]').first().click().type(informacion);
     // click en update
