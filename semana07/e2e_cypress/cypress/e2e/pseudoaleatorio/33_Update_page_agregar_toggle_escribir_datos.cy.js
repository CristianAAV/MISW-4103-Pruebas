describe('update pages y agegar campo markdown', () => {
    ...
    cy.get('p[data-koenig-dnd-droppable="true"] > span[data-lexical-text="true"]').first().click().type('{enter}');
     ...
     //escoger toggle
     cy.get('button.group[role="menuitem"][data-kg-card-menu-item="Toggle"]').click();
     //editar texto del toggle
     cy.get(':nth-child(2) > .relative > .rounded > .items-start > .mr-2 > .koenig-lexical > [data-kg="editor"] > .kg-prose > p').first().click().type(informacion);
     // click en update
