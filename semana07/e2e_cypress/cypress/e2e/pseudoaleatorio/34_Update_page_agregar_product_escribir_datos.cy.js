describe('update pages y agegar campo markdown', () => {
    ...
    cy.get('p[data-koenig-dnd-droppable="true"] > span[data-lexical-text="true"]').first().click().type('{enter}');
     ...
     //escoger toggle
     cy.get('div.flex.w-full.justify-between > div:contains("Product")').click();
     //editar texto del callout
     cy.get('.mr-2 > .koenig-lexical > [data-kg="editor"] > .kg-prose > p').first().click().type(informacion);
     // click en update
